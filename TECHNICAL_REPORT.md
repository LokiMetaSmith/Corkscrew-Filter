...
```mermaid
graph TD
    subgraph Controller ["Orchestrator (optimizer/main.py)"]
        Start(Start Optimization)
    end

    subgraph Generation ["1. Generation Phase"]
        SD[ScadDriver]
        JS[export.js / openscad-wasm]
        STL[(STL Geometry)]
    end

    subgraph CFD ["2. CFD Phase (OpenFOAM)"]
        FD[FoamDriver]
        SHM[snappyHexMesh]
        SF[simpleFoam]
        Res[Simulation Results / Metrics]
    end

    subgraph AI ["3. Feedback Loop"]
        Agent[LLMAgent]
        Gemini[Google Gemini API]
        Params[New Design Parameters]
    end

    Start --> Agent
    Agent -- Initial Params --> SD
    Params --> SD
    SD --> JS
    JS --> STL
    STL --> FD
...
![Figure 2: Wireframe view of Helical Geometry generated by OpenSCAD (Section 4.2).](images/Wireframe%20view%20of%20Helical%20Geometry.png)
...
*   **Unified Barb Module:** Hose retention geometry is now generated by a fully parameterized `Barb` module. This replaces hardcoded functions with a "Christmas tree" profile generator that dynamically calculates barb count, swell diameter, and wall thickness to match specific hose flexibility requirements.
...
7.  **Component Modularization:** The `Barb` generator and `FilterHolder` components have been fully parameterized and standardized, replacing hardcoded geometry.
