...
async function OpenSCAD(options) {
    if (!wasmModule) {
        const url = new URL(`./openscad.wasm.js`, import.meta.url);
        wasmModule = url.href;
    }
    const module = {
        noInitialRun: true,
    ...
    // Inject WASM directly to bypass node fetch issues
    if (!module.wasmBinary) {
        const fs = await import('fs');
        const wasmPath = new URL(`./openscad.wasm`, import.meta.url);
